...
namespace NewMMS.Contract.Domain
{
    ...
    public class IbanConvertor : IIbanConvertor
    {
        ...
        {
            if (accountNo.ToString().Length <= 10 && (branchId == null || branchId == 0)) 
                throw new Exception("كد شعبه براي حسابهاي غير متمركز الزامي مي باشد");
            if (accountNo.ToString().Length == 11 || accountNo.ToString().Length > 13)
                throw new Exception("شماره حساب صحيح نمي باشد");
            if (accountNo.ToString().Length > 11 && branchId != null && branchId > 0) branchId = 0;
            if (accountNo.ToString().Length > 11 && !IsValidSibaCheckDigit(accountNo))
                throw new Exception("شماره حساب  معتبر نمي باشد، رقم كنترلي  با شماره حساب متناسب نمي باشد");
        ...
        public static IbanStatusMessages IsValidIbanFormat(string iban)
        {
            ...
            if (iban.Substring(0, 2) != IranCode &&
                string.Equals(iban.Substring(0, 2), IranCode, StringComparison.CurrentCultureIgnoreCase))
                return IbanStatusMessages.InvalidIranIdMustBeUpperCh;
            if (iban.Substring(0, 2) != IranCode)
                return IbanStatusMessages.InvalidIranId;

            //if (IBAN.Substring(4, 3) != BMI_ID)
            //   return IBANStatusMessages.InvalidBankID;
            if (!IsDecimal(iban.Substring(2)))
                return IbanStatusMessages.InvalidAccountNumber;
            if (!IsValidControlDigit(iban))
                return IbanStatusMessages.InvalidControlDigit;
    ...
    public interface IIbanConvertor
    {
    }
